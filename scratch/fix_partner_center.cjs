...
function robustReplace(desc, startAnchor, endAnchor, replacement) {
    let i1 = content.indexOf(startAnchor);
    if (i1 === -1) throw new Error("Could not find start for " + desc);
    let searchStartPos = startAnchor.trim() === '' ? i1 : i1 + startAnchor.length;
    let i2 = content.indexOf(endAnchor, searchStartPos);
    if (i2 === -1) throw new Error("Could not find end for " + desc + " => " + endAnchor.substring(0, 50));
    content = content.substring(0, i1) + replacement + content.substring(i2 + endAnchor.length);
}

// 3. PartnerHome three-col layout to Feed Grid
const newPartnerHomeMiddle = \`      {/* Feed Column */}
...
robustReplace("PartnerHome Layout Top",
"      {/* Center Column - Feed */}\\r\\n      <div className=\\"ln-col-center\\">",
"      <div className=\\"ln-col-center\\">",
newPartnerHomeMiddle.replace(/\\n/g, '\\r\\n'));

// 4. Create Post Modal rewrite
const newPostModal = \`        {/* Create Post Trigger */}
...
robustReplace("Post Modal",
"        {/* Create Post Trigger */}",
"            </div>\\r\\n          </div>\\r\\n        )}",
newPostModal.replace(/\\n/g, '\\r\\n'));

// 5. Replace partner layout wrap endings
robustReplace("PartnerHome block end",
"      {/* Right Aside - Widgets */}",
"      )}",
"");

robustReplace("tt-three-col to standard dash", "<div className=\\"ln-three-col\\">", "<div className=\\"ln-three-col\\">", "<div>");

fs.writeFileSync(path, content, 'utf8');
console.log("Written center layout mod!");
