const fs = require('fs');
let content = fs.readFileSync('src/components/dashboards/PartnerDashboard.jsx', 'utf8');
                  ...
                  <div className="tt-feed-card-content">
                      <div className="tt-feed-card-title" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                          ...
                          {item.author_id === currentUser?.id && (
                              <button onClick={(e) => { e.stopPropagation(); if (window.confirm('Delete bulletin?')) deletePost(item.id); }} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer' }}>
                                  <Trash2 size={16} />
                              </button>
                          )}
                      </div>
                ...
                <div key={\`job-\${item.id}\`} className="tt-feed-card">
                  ...
                  <div className="tt-feed-card-content">
                      ...
                      <div className="tt-feed-card-footer" style={{ display: 'flex', gap: 8 }}>
                          {myJob && (
                              <button className="tt-feed-card-btn tt-feed-card-btn-primary" onClick={(e) => { e.stopPropagation(); setActivePage('applicants'); }}>
                                  <Users size={14} /> View Applicants
                              </button>
                          )}
                          <button className="tt-feed-card-btn" onClick={(e) => { e.stopPropagation(); openJobMediaModal(item, true); }}>
                              <MessageSquare size={14} /> Comment ({jobComments.length})
                          </button>
                          <button
                              className="tt-feed-card-btn"
                              onClick={(e) => { e.stopPropagation(); !myJob && openContactModal({ recipientId: item.partnerId, recipientType: 'industry_partner', recipientName: item.companyName, jobPostingId: item.id, sourceLabel: item.title })}}
                              disabled={myJob}
                          >
...
const postNew = `              return (
                <div key={\`post-\${item.id}\`} className="tt-feed-card">
                  ...
                  <button type="button" className="tt-feed-card-icon" onClick={(e) => { e.stopPropagation(); openProfile({ id: item.author_id, type: authorProfileType }); }} style={{ cursor: 'pointer', border: 'none', padding: 0, fontWeight: 700, color: '#334155' }}>
                  ...
                  <div className="tt-feed-card-content">
                              ...
                              <div style={{ position: 'relative' }}>
                                  ...
                                  {postMenuId === item.id && (
                                    <div style={{ position: 'absolute', right: 0, top: 24, background: '#fff', borderRadius: 8, boxShadow: '0 4px 12px rgba(0,0,0,0.15)', border: '1px solid #e2e8f0', zIndex: 10, minWidth: 150 }} onClick={e => e.stopPropagation()}>
                                      <button onClick={(e) => { e.stopPropagation(); setEditingPostId(item.id); setEditContent(item.content); setPostMenuId(null); }} style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '10px 14px', border: 'none', background: 'none', cursor: 'pointer', color: '#1e293b', textAlign: 'left' }}><Edit size={14}/> Edit post</button>
                                      <button onClick={(e) => { e.stopPropagation(); if(window.confirm('Delete this post?')) deletePost(item.id); setPostMenuId(null); }} style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '10px 14px', border: 'none', background: 'none', cursor: 'pointer', color: '#dc2626', textAlign: 'left' }}><Trash2 size={14}/> Delete post</button>
                                    </div>
                                  )}
                              </div>
                          )}
                          ...
                          {editingPostId === item.id ? (
                              <div style={{ marginTop: 8 }} onClick={e => e.stopPropagation()}>
                                  ...
                                  <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 8 }}>
                                      <button onClick={(e) => { e.stopPropagation(); setEditingPostId(null); setEditContent(''); }} style={{ padding: '6px 14px', borderRadius: 20, border: 'none', background: '#f1f5f9', color: '#475569', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
                                      <button onClick={(e) => { e.stopPropagation(); handleEditPost(item) }} disabled={!editContent.trim()} style={{ padding: '6px 14px', borderRadius: 20, border: 'none', background: '#0a66c2', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Save</button>
                                  </div>
                              </div>
                          ) : (
                            item.content
                      ...
                      <div className="tt-feed-card-footer" style={{ display: 'flex', gap: 8 }}>
                          <button className="tt-feed-card-btn" onClick={(e) => { e.stopPropagation(); handleCommentOnPost(item); }}>
                              Comment ({comments.length})
                          </button>
                          <button className="tt-feed-card-btn tt-feed-card-btn-primary" disabled={isMe} onClick={(e) => { e.stopPropagation(); openContactModal({ recipientId: item.author_id, recipientType: toRecipientAuthorType(item.author_type), recipientName: author?.name || author?.companyName || 'Unknown', postId: item.id, sourceLabel: item.content }); }}>
                              Contact
                          </button>
                      </div>
                  </div>
...
function doReplace(blockName, startStr, endStr, newStr) {
  const i1 = content.indexOf(startStr);
  if (i1 === -1) throw new Error("Could not find start for " + blockName);
  const i2 = content.indexOf(endStr, i1);
  if (i2 === -1) throw new Error("Could not find end for " + blockName);
  content = content.substring(0, i1) + newStr + content.substring(i2 + endStr.length);
}

// 1. replace bulletin block
// We know it starts right after:
// const reqs = Array.isArray(item.requirements) ? item.requirements : [];
let bullStart = "              return (\r\n                <div key={`bulletin-${item.id}`}";
if (content.indexOf(bullStart) === -1) {
    bullStart = "              return (\n                <div key={`bulletin-${item.id}`}";
}
let bullEnd = "              );\r\n            } else if (item.feedType === 'job') {";
if (content.indexOf(bullEnd) === -1) {
    bullEnd = "              );\n            } else if (item.feedType === 'job') {";
}
// since end contains } else if (item.feedType === 'job') {, we MUST preserve it by subtracting it or just appending it
const bullReplace = bulletinNew + "\n            } else if (item.feedType === 'job') {";
doReplace("bulletin", bullStart, bullEnd, bullReplace);


// 2. replace job block
let jobStart = "              return (\r\n                <div key={`job-${item.id}`}";
if(content.indexOf(jobStart) === -1) jobStart = "              return (\n                <div key={`job-${item.id}`}";
let jobEnd = "              );\r\n            } else {";
if(content.indexOf(jobEnd) === -1) jobEnd = "              );\n            } else {";
const jobReplace = jobNew + "\n            } else {";
doReplace("job", jobStart, jobEnd, jobReplace);


// 3. replace post block
let postStart = "              return (\r\n                <div key={`post-${item.id}`}";
if(content.indexOf(postStart) === -1) postStart = "              return (\n                <div key={`post-${item.id}`}";
let postEnd = "              );\r\n            }\r\n          })}";
if(content.indexOf(postEnd) === -1) postEnd = "              );\n            }\n          })}";
const postReplace = postNew + "\n            }\n          })}";
doReplace("post", postStart, postEnd, postReplace);

fs.writeFileSync('src/components/dashboards/PartnerDashboard.jsx', content, 'utf8');
console.log("Successfully ran correct regex replacements!");
